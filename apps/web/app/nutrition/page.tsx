...
export default async function NutritionPage({ searchParams }: { searchParams: Promise<{ days?: string }> }) {
  ...
  const [trendRaw, meals, settings] = await Promise.all([healthApi<Trend[]>(`/v1/nutrition/trend?days=${days}`), healthApi<Meal[]>("/v1/meals/recent?limit=40"), healthApi<Settings>("/v1/settings")]);
  ...
  return <div className="page"><header className="page-header compact"><div><span className="kicker">ROLLING INTAKE / {days} DAYS</span><h1>Nutrition<br /><em>history.</em></h1></div><div className="range-switch"><a className={days === 7 ? "active" : ""} href="/nutrition?days=7">07D</a><a className={days === 30 ? "active" : ""} href="/nutrition?days=30">30D</a></div></header>
    <section className="metric-grid two"><Metric label="AVG ENERGY" value={calorieAverage} unit="kcal/day" accent hint={`${Math.round((calorieAverage / settings.calorieTarget) * 100)}% of target`} /><Metric label="AVG PROTEIN" value={proteinAverage} unit="g/day" hint={`${Math.round((proteinAverage / settings.proteinTargetG) * 100)}% of target`} /></section>
    ...
    <section className="panel"><div className="panel-title"><span>03 / MEAL ARCHIVE</span><strong>{meals.length} recent</strong></div><MealList meals={meals} timeZone={settings.timezone} /></section>
  </div>;
}
