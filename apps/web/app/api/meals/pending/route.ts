...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await healthApiRequest<{ pending: unknown | null }>(
      `/v1/meals/pending/latest?scopeKey=${encodeURIComponent(scopeKey)}`,
      {
        method: "GET",
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await healthApiRequest("/v1/meals/pending", {
      method: "POST",
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to create pending meal" },
      { status: 500 },
    );
  }
}
